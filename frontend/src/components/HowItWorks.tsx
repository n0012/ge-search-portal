...
const SECTIONS: { src?: string; title: string; body: string }[] = [
  ...
  },
  {
    ...
    body: "AI is opt-in (search stays fast and LLM-free by default), and every answer grounds only on the user's ACL-trimmed documents. Answers are generated by the Gemini Enterprise engine assistant (:streamAssist) — query understanding, retrieval, grounded generation, and inline citations, all server-side — with the authorized doc-id set passed as a filter so the ACL trim holds. The same path powers the result-set answer, follow-up Q&A, and per-document Q&A. Because all calls (search + answer) go through the GE engine, they draw on the pooled per-seat subscription rather than standalone pay-as-you-go charges.",
  },
  {
