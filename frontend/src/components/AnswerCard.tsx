...
import type { AnswerMeta, Citation } from "../types";
...
/** AI answer card. Opt-in: idle (with a Generate button) → loading skeleton → markdown answer
 *  + citations. Every answer is generated by the Gemini Enterprise engine assistant, grounded on
 *  the user's ACL-trimmed documents. Fetched separately from search so results stay fast. */
export function AnswerCard({
  ...
  requested: boolean;
  onGenerate: () => void;
  onAsk?: (question: string, session?: string) => Promise<import("../api").AskResult>;
}) {
    ...
    <div className="rounded-2xl border border-amgen-blue/20 bg-gradient-to-br from-amgen-blue/[0.04] to-white p-5 shadow-card">
      ...
      {idle ? (
        ...
        <>
          ...
          {meta && summary && (
            <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-amgen-line pt-2 text-[11px] text-amgen-muted">
              <span title="Answers are generated by the Gemini Enterprise engine assistant; the underlying model is managed by the engine and not disclosed per response.">
                Gemini Enterprise · {meta.assistant}
              </span>
              <span
                title="Estimated from answer length (~4 chars/token). The GE assistant doesn't meter tokens — usage is covered by the per-seat subscription, not billed per token."
              >
                ≈{meta.tokensEstimated.toLocaleString()} tokens (est.)
              </span>
